...
/**
 router未注册回调
 */
@property (nonatomic,strong) MGJRouterUnregisterURI routerUnregisterURLHandler;
...
/// 这里可以作为检查是否有没有注册，最后都是执行注册传进来的block代码，从block入手
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        ...
        ///没有实现注册会走
        if (handler == NULL) {
            NSString *routerUrl = parameters[MGJRouterParameterURL];
            [[self sharedInstance] unregisterURLBeRouterWithURL:routerUrl];
        }
        
        /*
        打印 parameters 参数
         
        MGJRouterParameterURL = "TM://ModeJ/Push_TMModeJViewController";
        MGJRouterParameterUserInfo =     {
            MGJRouterParameterNavigation = "<UINavigationController: 0x7f9bbf01e800>";
            backImage = "<UIImage: 0x6000000498f0>, {500, 500}";
            block = "<__NSGlobalBlock__: 0x10fa71470>";
            title = "ModeF\U6807\U9898";
        };
        block = "<__NSGlobalBlock__: 0x10fa713b0>";
         */
    }
}
...
/// 未注册的实现
+ (void)routeUnregisterURLHandler:(MGJRouterUnregisterURI)handler{
